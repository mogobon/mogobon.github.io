...
:::{.note}
* この記事は`r Sys.Date()`現在執筆途中で未完成です. 内容・構想は流動的に変わります. 
...
:::{.assumption #cassump name="尺取法が成立するための, 仮定"}
...
* <font color=gray>$c^{op}$は最適([op]timal)な$c$を, $len^{op}$は最適な長さ([len]gth)を表す. </font>
* $c^{op}\leftarrow e,~c\leftarrow e,~l\leftarrow 0,~r\leftarrow 0,~len^{op}\leftarrow0$
* **<font color = red>for</font>** $l$ **<font color = red>in</font>** $0,1,\cdots,N-1$:
  * <font color=gray>// $r$が探索範囲に含まれており, かつ条件を満たす間$r$を一つずつ右に動かしていき, $r_{l}$を求める.</font>
  * **<font color = red>while</font>** $r<N\land is\_valid(\mathrm{add}(c(l,r),s_{r}))==True$:
    * $c\leftarrow \mathrm{add}(c(l,r),s_{r})$
    
    <!--* ($\mathrm{add}(c(l,r),s_{r})= c\oplus s_{r}$となる場合もある.)-->
    
    * $r\leftarrow r+1$
  * <font color=gray>// 条件を満たさなくなったら, 答えを現在の答えと求めた$c(l,r_{l})$のうち, 最適な方に更新する.</font>
  * $c^{op}\leftarrow f(c^{op},~c)$
  * $len^{op}\leftarrow g(len^{op},~r-l)$
    * <font color=gray>$f,g$は比較関数の記号である.</font>
  * <font color = gray>// $c(l,r_{l})$と$s_{l}$から, $c(l+1,r_{l+1})$の初期値を求める.</font>
  * $c(l+1,r)\leftarrow \mathrm{remove}(c(l,r_l),s_{l})$
  
  <!--* ($\mathrm{remove}(c(l,r_l),s_{l})=c(l,r_l)\oplus s_{l}^{-1}$となる場合もある.) -->
...
台集合$S=\{s_0,\cdots, s_{N-1}\}$と, $S$上の二項演算$\oplus$の組$(S,\oplus)$は,次の性質を満たす. 
...
列$S'$を$S'=(s_0,s_1,\cdots,s_{N-1})$,  $c(L,R)=\displaystyle\bigoplus_{i=L}^{R-1} s_{i}\equiv s_{L}\oplus s_{L+1}\oplus s_{R-1}$とする. この$c$を用いた尺取り法は, $\mathrm{add}(c(l,r),s_{r})=c(l,r)\oplus s_{r}$, $\mathrm{remove}(c(l,r),s_{l})=s_{l}^{-1}\oplus c(l,r) $と置き換えて次のように表現できる.

* $c^{op}\leftarrow e,~c\leftarrow e,~l\leftarrow 0,~r\leftarrow 0,~len^{op}\leftarrow0$
* **<font color = red>for</font>** $l$ **<font color = red>in</font>** $0,1,\cdots,N-1$:
  * **<font color = red>while</font>** $r<N\land is\_valid(c\oplus s_{r})==True$:
    * $c\leftarrow c\oplus s_{r}$
    * $r\leftarrow r+1$
  * $c^{op}\leftarrow f(c^{op},~c)$
  * $len^{op}\leftarrow g(len^{op},~r-l)$
  * $c(l+1,r)\leftarrow s_{l}^{-1}\oplus c(l,r)$
  
  <!--* $l\leftarrow l+1$    -->

:::
<details><summary>例 \@ref(exm:Groupexm) の差分更新が高速できることの証明</summary>
:::{.proof name="差分更新の効率性"}

<!--* $s_i>0$より, $c(l,r)=\displaystyle\sum_{i=l}^{r-1}s_i$, $c(l,r)=\displaystyle\prod_{i=l}^{r-1}s_i$は明らかに区間の長さに対して単調である. -->

* 例 \@ref(exm:Groupexm) において, 差分更新は高速に行うことができることを以下で示す.

* $c(l,r+1)= \mathrm{add}(c(l,r),s_{r+1})$が高速に計算できることを示す.

<!--* $c(L,R)=\displaystyle\bigoplus_{i=L}^{R-1}s_i$と表記する. -->
