...
* このもとで, 差分更新は高速に行うことができることを以下で示す.
...
* $c(l,r+1)$は累積代入により, 求めることができることがわかる.
  * 例(組$(S,\times),~S\subset\mathbb{Z}\setminus\{0\}$の場合): `c *= s[r]` 
...
* また, 組$(S,\oplus)$が, 可換律$x,y\in S, x\oplus y=y\oplus x$を満たしていれば, $c(l+1,r)$は累積代入により, 求めることができることがわかる.
    * 例(組$(S,\times),~S\subset \mathbb{Z}\setminus\{0\}$の場合): `c //= s[l]`
...
* **適切な仮定のもとで**, 上の尺取り法において, $\mathrm{add}(c(l,r),s_{r})=c(l,r)\oplus s_{r}$, $\mathrm{remove}(c(l,r),s_{l})=c(l,r)\oplus s_{l}^{-1}$と置き換えたとしても, 動作するという仮説を考えている.
* これを下に示す. 


:::{.hypothesis}
列$S'$を$S'=(s_0,s_1,\cdots,s_{N-1})$とする. 台集合$S$を, $S=\{s_0,s_1,\cdots,s_{N-1}\}$とする. 
$c(L,R)=\displaystyle\bigoplus_{i=L}^{R-1} s_{i}\equiv s_{L}\oplus s_{L+1}\oplus s_{R-1}$とする. 組$(S,\oplus)$がアーベル群であり, さらに$x\oplus y~(x,y\in S)$が高速($O(1)$程度を想定)に動くとき, 次の尺取り法アルゴリズムは正当である. 
...
* $l\in[0,N)$の間繰り返す. 
  ...
  * $c^{op}\leftarrow f(c^{op},~c)$
  * $len^{op}\leftarrow g(len^{op},~r-l)$
  * $c(l+1,r)\leftarrow c(l,r)\oplus s_{l}^{-1}$
  * $l\leftarrow l+1$
...
* このブログは書きかけです。今後更新するかもしれないことをまとめています。
  ...
  * $s(I)$の定義と演算の範囲
  * 他の問題の例も試してミスの穴を埋めたい
* 群論については, 勉強したてで準同型定理や剰余群すら怪しいです.（ズコー！）
